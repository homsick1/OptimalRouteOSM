...
$(document).ready(function (e) {
    ...
    //
    var defaultKey = "0d9bd7be-5e31-4f2b-a295-ea71e82dfa70";
    var profile = "car";
    ...
    map.on('click', function (e) {
        ...
        if (ghRouting.points.length > 1) {
            ...
            ghRouting.doRequest()
                ...
                .catch(function (err) {
                    var str = "Произошла ошибка: " + err.message;
...
function setupRouteOptimizationAPI(map, ghOptimization, ghRouting) {
    ...
    var optimizeError = function (err) {
        ...
        if (err.message.indexOf("Too many locations") >= 0) {
            ...
            $("#vrp-error").text("Произошла ошибка: " + err.message);
        }
        console.error(err);
    };
    ...
    var optimizeResponse = function (json) {
        ...
        $("#vrp-response").text("Найдено решение для " + sol.routes.length + " машин(ы)! "
            + "Расстояние: " + Math.floor(sol.distance / 1000) + "км "
            + ", время: " + Math.floor(sol.time / 60) + "мин "
            + ", стоимость в попугаях: " + sol.costs);
        ...
        for (var routeIndex = 0; routeIndex < sol.routes.length; routeIndex++) {
            ...
            ghRouting.doRequest({instructions: false})
                .then(ghCallback)
                .catch(function (err) {
                    var str = "Произошла ошибка для маршрутизации: " + err.message;
                    $("#vrp-error").text(str);
                });
        }
    };
    ...
    var optimizeRoute = function () {
        if (ghOptimization.points.length < 3) {
            $("#vrp-response").text("Для построения маршрута требуется минимум 3 точки. Вы указали всего: " + ghOptimization.points.length);
            return;
        }
        $("#vrp-response").text("Построение маршрута ...");
...
function setupGeocodingAPI(map, ghGeocoding) {
    ...
    var mysubmit = function () {
        ...
        ghGeocoding.doRequest({query: textField.val()})
            ...
            .catch(function (err) {
                $("#geocoding-error").text("Произошла ошибка: " + err.message);
            });
    };
    ...
    map.on('click', function (e) {
        ...
        if (!inprogress) {
            ...
            ghIsochrone.doRequest({point: pointStr, buckets: 2})
                ...
                .catch(function (err) {
                    inprogress = false;
                    $('#isochrone-response').text("Произошла ошибка: " + err.message);
                })
            ;
        } else {
